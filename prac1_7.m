...
Fx = (0.0:0.1:2);                  % kept lower for initial testing
...
for f = 1:length(Fx)    % vary force applied
    for t = 1:Tmax      % vary starting configs
        ...
        while i<=naccept   % MC loop
            ...
            % polymer2(:,bm:end) = polymer(:,bm:end) - polymer(:,bm) + newbond(:);

            % pass/fail parameters
            Q1 = (polymer(1,end)-polymer(1,1));
            Q2 = (polymer2(1,end)-polymer2(1,1));
            V1 = -Fx(f)*Q1;
            V2 = -Fx(f)*Q2;
            dv = V2-V1;
...
% fprintf('\nAverage extention at temperature  %d over %d configurations = %d\n',Temp,Tmax,Qxnum)

figure
plot(Q)
grid on
title(sprintf('force-extension for Fx=%d',Fx(f)))
